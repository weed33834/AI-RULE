...
设计哲学：
- Rule Hub 是"规则源"不是 agent framework，不包装 LLM 调用
- 但 AI 可以在交付前主动调 `ai-rule check-output` 校验自己的输出
- 失败时给 AI 一个明确的"哪里错了 + 怎么修"反馈，触发自动重写

两种使用方式：

1. CLI 用法（agent 主动调用，推荐）：
   ai-rule check-output --profile coding --output-type code_change --content "$(cat my_patch.diff)"

2. Python API 用法（编程式集成，供其他工具调用）：
   from ai_rule.output_schemas import validate_output, CodeChangeSchema
   result = validate_output(CodeChangeSchema, content)

Schema 定义来自 manifests/<profile>.yaml 的 output_schemas 段（可选）。
...
# 原因：保持 ai-rule 零硬依赖（与 sync_rules.py 一致）
