...
设计哲学：
- Rule Hub 是"规则源"不是 agent framework，不强制用户装任何沙箱依赖
...
Python API：
    from ai_rule.sandbox import run_in_sandbox
    result = run_in_sandbox("python test_auth.py", cwd="/path/to/project")

CLI 用法（agent 主动调）：
    ai-rule sandbox-run --command "python test_auth.py" --cwd /path/to/project
"""
