...
本仓库是一套**通用 AI 编程助手规则模板**。`AGENTS.md` 是所有规则的**唯一源头**，其余工具配置文件由同步脚本生成，请勿手工修改。
...
2. 运行同步脚本，自动重生成工具文件：
   ...
   脚本会把 `AGENTS.md` 中的 `@引用` 内联展开，重新生成 `CLAUDE.md`、`GEMINI.md`、`.github/copilot-instructions.md`。
3. 提交时**不要手工改动** `CLAUDE.md` / `GEMINI.md` / `copilot-instructions.md` —— 它们由脚本生成，手改会被下次同步覆盖，并造成源头与派生文件不一致。
...
- MCP 相关变更只改说明文本，**绝不添加自动下载或自启动指令**（见 `AGENTS.md` §5 红线）。
