...
**五个首发场景规则包，随场景自动路由。** `coding`（默认）、`conversation`、`novel`、`paper`、`agent-builder` 面向不同任务场景——每个规则包 = 场景协议 + 提示词 + 技能 + 能力白名单，由项目锚点与用户意图自动选择，也可 `agentseed switch --profile novel` 手动切换。规则包可插拔、可扩展：新增场景只需一个目录 + 一份清单，机制开放，后续场景持续加入，全程不需要改内核。
