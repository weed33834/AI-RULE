...
> 读取方式：AI 在选工具时先检索本表；无匹配项才走「受限自主搜索」协议（见 AGENTS.md § Skill Acquisition Protocol）。
...
> 以下不是「直接安装的工具」，而是**学习 / 选型灵感来源**。需要落地某框架时，仍走 § Skill Acquisition Protocol 的受限搜索协议评估，优先厂商官方仓库与高星可信源。
