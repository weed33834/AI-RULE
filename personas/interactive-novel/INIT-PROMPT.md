...
- The 25 skill documents under `skills/` and 4 prompts under
  `prompts/` are present.
...
2. Read every skill document it references via `@skills/*.md`.
3. Read the four engine prompts under `prompts/`:
...
8. **Difficulty** — starting difficulty band
   (see `@skills/difficulty-engine.md`).
...
5. **Per session end:** run `@skills/game-evaluation.md`, update memory,
...
- If context budget is exceeded, follow `@skills/context-management.md`
  before continuing.
