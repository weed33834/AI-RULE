...
- 记忆系统架构详见 `skills/memory-systems.md`，上下文工程详见 `skills/context-engineering.md`。
...
- For memory system architecture, see `skills/memory-systems.md`. For context engineering, see `skills/context-engineering.md`.
...
> 当知识图谱记忆层启用后，检索策略可进一步升级。参考 `skills/advanced-patterns.md` 中的模式 11：
...
> When the knowledge graph memory tier is enabled, retrieval strategy can be further upgraded. Reference pattern 11 in `skills/advanced-patterns.md`:
