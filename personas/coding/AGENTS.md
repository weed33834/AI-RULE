...
- 远程沙箱工程规范：在远程沙箱/CI 环境开发时必须查阅 @personas/coding/skills/remote-sandbox-sop.md（工作目录选择、push 时机、命令输出可见性、subagent 拆分等通用教训）。
...
- Git 操作前必须查阅: @personas/coding/skills/git-sop.md
...
- 复杂脚本/工具必须查阅授权白名单: @personas/coding/skills/registry.md
- 若需从 GitHub 下载脚本，必须先展示 URL 和 Star 数，经用户同意后下载至临时目录，审查后使用。
- 获取层级（标准库 → 包管理器 → 本地注册表 → 优先厂商官方仓库 → 受限自主搜索）：详见 @personas/coding/skills/registry.md。
...
- 允许的 MCP 服务清单与配置说明见 @personas/coding/skills/mcp-registry.md（仅参考，手动配置）。
- 三者关系与落地结构详解见 @personas/coding/skills/tool-skill-mcp.md。
...
## References
- 智能体提示词: @personas/coding/prompts/system-prompt.md
- 架构师角色: @personas/coding/prompts/architect-subagent.md
- 工程师角色: @personas/coding/prompts/engineer-subagent.md
- 审查官角色: @personas/coding/prompts/critic-subagent.md
- 验证员角色: @personas/coding/prompts/verifier-subagent.md
- 交付角色: @personas/coding/prompts/final-subagent.md
- 技能注册表: @personas/coding/skills/registry.md
- 远程沙箱工程规范: @personas/coding/skills/remote-sandbox-sop.md
