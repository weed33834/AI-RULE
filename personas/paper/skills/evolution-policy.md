...
| draft | Proposal accepted by the user | Author the skill file under `skills/`; tag with version `0.x` |
| active | Draft passes one review cycle with positive feedback | Promote to version `1.0`; add to the active skill index |
| deprecated | Skill is obsolete, redundant, or replaced | Move to `skills/archive/`; record replacement in CHANGELOG |
