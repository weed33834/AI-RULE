...
def generate_hooks(platform_id: str, hook_dir: str) -> dict:
    ...
    在 adapters/hooks/<platform_id>/ 下创建:
      - pre_tool_use.py       (pre-tool-use 拦截脚本)
      - hooks.json.template   (Hook 配置模板)
      - README.md             (安装指引)

    返回 {files_created: [路径...], hook_dir: 目录路径}。
    """
    from . import sync_rules as _sr
    ...
    if not tool_script.exists():
        py_src = '''"""AgentSeed {name} PreToolUse Hook — 用户自定义拦截脚本。

安装步骤：见同目录 README.md。
核心逻辑：调用 adapters/hooks/shared/check.py 的 decide() 函数。
约束源：core/constraints.yaml（由 agentseed forge --emit-constraints 自动加载到项目）。
"""
from __future__ import annotations
import json, sys
from pathlib import Path
...
sys.path.insert(0, str(_HOOKS_DIR / "shared"))
...
def main():
    try:
        ...
        print(json.dumps({{
            "allow": True,
            "reason": f"Hook error (fail-open): {{e}}",
        }}))

if __name__ == "__main__":
    main()
'''.format(name=platform_id)
        tool_script.write_text(py_src, encoding="utf-8")
    ...
    if not template.exists():
        json_src = '''{{
  "_comment": "{name} Hook 配置模板。参考同目录 README.md 安装指引。",
  "_setup": "1) 确保 adapters/hooks/ 目录在项目根 2) 复制本模板为 hooks.json 或 settings.json（按平台要求）3) 修改 command 路径指向 pre_tool_use.py",
  "preToolUse": [
    {{"command": "python ./adapters/hooks/{pid}/pre_tool_use.py"}}
  ]
}}
'''.format(name=platform_id, pid=platform_id)
        template.write_text(json_src, encoding="utf-8")
        created.append(template)

    # 3. README.md
    readme = hooks_root / "README.md"
    if not readme.exists():
        md_src = '''# {name} PreToolUse Hook

AgentSeed 自动生成的 {name} 平台 PreToolUse 钩子模板。
...
1. **确保 AgentSeed 已安装**：
   ...
   或从源码：
   ```bash
   git clone https://github.com/agentseed/agentseed.git
   pip install -e .
   ```

2. **复制钩子配置**：
   将 `hooks.json.template` 重命名为你的平台要求的配置文件名，
   放到钩子目录（`{hook_dir}/`）下。

3. **修改 command 路径**：
   确保 `command` 指向正确的 `pre_tool_use.py` 路径。

4. **测试**：
   在项目里执行 `agentseed forge`，会同时同步规则 + 钩子配置。

## 工作原理

`pre_tool_use.py` 读取平台传来的工具调用信息，调用
`adapters/hooks/shared/check.py` 的 `decide()` 函数。

`decide()` 从 `core/constraints.yaml` 读取约束并决策：
- P0 红线 → 直接拒绝
- P1 警告 → 允许但输出警告
- 其他 → 放行
...
你可以修改 `pre_tool_use.py` 添加自定义拦截逻辑。
核心入口是 `decide(tool_name, tool_input)` 函数。

## 故障恢复

如果钩子出问题导致无法使用：
1. 重命名/删除 `hooks.json`（或平台对应的配置文件）
2. 重启 AI 工具
3. 钩子是 fail-open 设计：脚本出错默认放行
'''.format(name=platform_id, pid=platform_id, hook_dir=hook_dir)
        readme.write_text(md_src, encoding="utf-8")
